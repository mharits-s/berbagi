    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                <span class="text-white font-bold bg-green-500 rounded-2xl w-fit p-4">
                    Fundraising telah aktif dan dapat menerima donasi.
                </span>
                @else
                <div class="flex flex-row justify-between">
                    <span class="text-white font-bold bg-red-500 rounded-2xl w-fit p-4">
                        Fundraising belum disetujui oleh SuperAdmin (owner).
                    </span>
                    @role('owner')
                    <form action="{{route('admin.fundraising.activate_fundraising', $fundraising)}}" method="POST">
                        @csrf                       
                        <button type="submit" class="font-bold gap-x-2 py-4 px-6 bg-indigo-700 text-white rounded-full flex justify-center items-center">
                            <lord-icon
                ...
                <div class="item-card flex flex-row gap-y-10 justify-between items-center">
                    ...
                    <div class="flex flex-row items-center gap-x-3">
                        <a href="{{route('admin.fundraisings.edit', $fundraising)}}" class="font-bold gap-x-2 py-4 px-6 bg-indigo-700 text-white rounded-full flex justify-center items-center">
                            <lord-icon
                        ...
                        <form action="{{route('admin.fundraisings.destroy', $fundraising)}}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="font-bold gap-x-2 py-4 px-6 bg-red-700 text-white rounded-full flex justify-center items-center">
                            <lord-icon
