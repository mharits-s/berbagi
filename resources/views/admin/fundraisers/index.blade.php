        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                <div class="item-card flex flex-row justify-between items-center">
                    ...
                    <span class="w-fit text-sm font-bold py-2 gap-x-2 px-4 rounded-full bg-green-500 text-white flex justify-center items-center">
                    <lord-icon
                    ...
                    <div class="hidden md:flex flex-row items-center gap-x-4">
                        <form action="#" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="font-bold py-3 gap-x-2 px-6 bg-red-700 text-white rounded-full flex justify-center items-center">
                            <lord-icon
                    ...
                    @else
                    <span class="w-fit text-sm font-bold py-2 gap-x-2 px-4 rounded-full bg-orange-500 text-white flex justify-center items-center">
                    <lord-icon
                    ...
                    <div class="hidden md:flex flex-row items-center gap-x-4">
                        <form action="{{route('admin.fundraisers.update', $fundraiser)}}" method="POST">
                            @csrf
                            @method('PUT')
                            <button type="submit"  class="font-bold py-3 gap-x-2 px-6 bg-indigo-700 text-white rounded-full flex justify-center items-center">
                            <lord-icon
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col ">
                <div class="item-card flex flex-col justify-between items-center gap-y-5">
                    ...
                    @if($fundraiserStatus == 'Pending')
                        <span class="w-fit text-sm font-bold  gap-x-2  py-3 px-6 rounded-full bg-orange-500 text-white flex justify-center items-center">
                        <lord-icon
                            src="https://cdn.lordicon.com/pxwxddbb.json"
                            trigger="loop"
                            state="loop-build-up"
                    ...
                    @elseif($fundraiserStatus == 'Active')
                        <a href="{{route('admin.fundraisings.create')}}" class="font-bold py-3 px-6 bg-indigo-700 text-white rounded-full">
                            Create Fundraising
                        </a>
